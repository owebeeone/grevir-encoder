...
#include <grevir/time/interactive_scaling.hpp>

namespace quad {

/**
 * Decodes a quadrature encoder and returns an absolute position.
 * Pin types supply static get() returning the physical level. High reads as
 * a zero bit and low as a one bit, matching the original polarity.
 */
...
/**
 * A scaler for interactivity. Clock supplies TimeType and now(). The
 * inherited min/max constructor arguments are unused: Time's scaler keeps
 * its built-in 4/100 period bounds.
 */
template <typename Clock>
using NonlinerarScaler = setl::RelativeInteractiveScaler<
    long, float, typename Clock::TimeType>;

template <typename Clock>
class InteractiveScaler : public NonlinerarScaler<Clock> {
public:
  InteractiveScaler()
    : NonlinerarScaler<Clock>{ 1.0f, setl::Period<float>(100), setl::Period<float>(500) }
  {}

  long scale(long value) {
    return this->scaleForNow(value, Clock::now());
  }
};

/**
 * The quadrature encoder class. This uses two pin types that provide static
 * get() and Core resource claims.
 */
template <
  typename w_PinA,
  typename w_PinB,
